...
def fast_merge_calls(
    ...
    update_job = hail_batch.get_batch().new_bash_job('Update VCF content')
    update_job.image(config.config_retrieve(['workflow', 'driver_image']))
    ...
    third_job.image(config.config_retrieve(['images', 'bcftools']))
    ...
    hail_batch.get_batch().write_output(third_job.output, str(output_path).removesuffix('.vcf.bgz'))
